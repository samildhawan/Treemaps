    ...
    *exactly* the same, so this doctest _might_ not pass when run on
    ...
    # Recursive case: path is a directory
        ...
        return (os.path.basename(path),
                [(filename, os.path.getsize(os.path.join(path, filename)) + 1)
                 if os.path.isfile(os.path.join(path, filename)) else
                 (filename, path_to_nested_tuple(os.path.join(path, filename))
                 [1]) for filename in ordered_listdir(path)])
...
def dir_tree_from_nested_tuple(obj: tuple[str, int | list]) -> DirectoryTree:
    ...
    # recursive case
        ...
        children = []
        for child in obj[1]:
            children.append(dir_tree_from_nested_tuple(child))
        return DirectoryTree(obj[0], children)
...
def _helper_moves_to_nested_dict(temp: dict) -> dict:
    """Return a nested dictionary representing the same move sequence from
    <temp>."""
    if not temp:
        return {}
    result = {}
    for move, subtree in temp.items():
        count = subtree.pop(None, 0)
        result[(move, count)] = _helper_moves_to_nested_dict(subtree)
    return result


def add_move(temp: dict, move: str) -> None:
    """Add the key <move> to <temp> with an empty dictionary as its value.
    If <move> already exists as a key in <temp>, no changes are made."""
    if move not in temp:
        temp[move] = {}


def add_result(temp: dict) -> None:
    """Add the key None to <temp> with a value of 1 if the dictionary is empty.
    If the dictionary is not empty, increment the value of None by 1."""
    if not temp:
        temp[None] = 1
    else:
        temp[None] = temp.get(None, 0) + 1
...
def moves_to_nested_dict(moves: list[list[str]]) -> dict[tuple[str, int], dict]:
    ...
    sequence_of_moves = {}
    for game in moves:
        temp = sequence_of_moves
        for i, move in enumerate(game):
            add_move(temp, move)
            temp = temp[move]
            if i == len(game) - 1:
                add_result(temp)
    return _helper_moves_to_nested_dict(sequence_of_moves)
...
class TMTree:
    ...
    def __init__(self, name: str, subtrees: list[TMTree],
        ...
        for subtree in self._subtrees:
            total_subtree_size += subtree.data_size
            subtree._parent_tree = self
        self.data_size = data_size + total_subtree_size
        # Set this tree as the parent for each of its subtrees.
        self._parent_tree = None
        # The tree is initially expanded, unless it has no subtrees.
        self._expanded = True if self._subtrees else False
        # This tree is initially a root (has no parent).
        for subtree in self._subtrees:
            subtree._parent_tree = self
    ...
    def _get_path_string_helper(self) -> str:
        ...
        if self._parent_tree is None:
            ...
            path_string = (self._parent_tree._get_path_string_helper()
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        if not self._subtrees:
            ...
            total_size_of_subtrees = 0
            for subtree in self._subtrees:
                total_size_of_subtrees += subtree.data_size
            if width > height:
                for subtree in self._subtrees[:-1]:
                    subtree_width = math.floor(
                        (width * subtree.data_size) / total_size_of_subtrees)
                    subtree.update_rectangles((x, y, subtree_width, height))
                    x += subtree_width
                subtree_width = width - x
                self._subtrees[-1].update_rectangles(
                    (x, y, subtree_width, height))
            else:
                for subtree in self._subtrees[:-1]:
                    subtree_height = math.floor(
                        (height * subtree.data_size) / total_size_of_subtrees)
                    subtree.update_rectangles((x, y, width, subtree_height))
                    y += subtree_height
                subtree_height = height - y
                self._subtrees[-1].update_rectangles(
                    (x, y, width, subtree_height))
        self.rect = rect
    ...
    def get_rectangles(self) -> \
        ...
        (0, 50, 100, 150)
        """
        if self.data_size == 0:  # Base Case: Tree does not have any data
            # associated with it, therefore immediately return an empty list
            # because there are no rectangles to draw.
            return []
        elif not self._subtrees:  # Second base case: subtrees is empty.
            # In this case, the root is within the displayed-tree, is a leaf,
            # and returned should only be a one rectangle as it has no children
            return [(self.rect, self._colour)]
        elif self.rect is None:  # Third Base case: Non-appropriate
            # pygame rectangle to display for a leaf
            return []
        else:
            lst = []
            if self.is_displayed_tree_leaf():  # Every leaf in the
                # displayed-tree rooted at this tree
    ...
    def get_tree_at_position(self, pos: tuple[int, int]) -> Optional[TMTree]:
        ...
        True
        """
        # Base Case: <pos> is outside this tree's rectangle, or Tree does not
        # have any data associated with it, so return None
        if (self.rect[0] > pos[0] or pos[0] > self.rect[0] + self.rect[2]) or (
                self.rect[1] > pos[1] or pos[1] > self.rect[1] + self.rect[3]) \
                or self.data_size == 0:
            return None
        # Second Base case: subtrees is empty.
        # In this case, the root is within the displayed-tree, is a leaf,
        # and returned should only be self.
        elif not self._subtrees:
            return self
        else:
            # List for potential rectangles that would contain <pos>
            subtree_list = []
            for subtree in self._subtrees:
                # Checks if <pos> is not None, and is a leaf in the
                # displayed-tree
                if (subtree.get_tree_at_position(pos) is not None
                        and subtree.get_tree_at_position(
                            pos).is_displayed_tree_leaf()):
                    # If a rectangle contains the position,
                    # and the returned tree is a leaf, it is added to the list
                    subtree_list.append(subtree.get_tree_at_position(pos))
            # In the case of ties/multiple ties when the <pos> is on the
            # boundary of two OR MORE rectangles, (edges, corners, boundaries)
            # return the first leaf of the  displayed-tree that contains
            # the position, when traversing the tree in the natural order. If
            # length of list is just 1, return leaf
    ...
    def expand(self) -> TMTree:
        ...
        if not self._subtrees:
            ...
            self._expanded = True
            first_subtree = self._subtrees[0].expand()
    ...
    def expand_all(self) -> TMTree:
        ...
        if not self._subtrees:
            ...
            last_subtree = None
            for subtree in self._subtrees:
                last_subtree = subtree.expand_all()
    ...
    def collapse_all(self) -> TMTree:
        ...
        """
        node = self
        while node._parent_tree is not None:
            node = node.collapse()

        # Set the _expanded attribute of all children of the root to False
    ...
    def move(self, destination: TMTree) -> None:
        ...
        True
        """
        # remove self from its parent's subtree list

        # Appropriately update the data_size attribute of ALL
        # TMTrees whose size changes as a result of this change.
        self._parent_tree._expanded = True
        new_parent_tree = self._parent_tree
        new_parent_tree._subtrees.remove(self)
        while new_parent_tree is not None:
            new_parent_tree.data_size -= self.data_size
            new_parent_tree = new_parent_tree._parent_tree
        # Expand self's new parent so that self remains a leaf in the
        # displayed-tree (self's new parent will no longer be a leaf in the
        # displayed-tree)
        ...
        while destination:
            ...
            destination = destination._parent_tree
        # Reapply the treemap algorithm to the root of the tree that self is
        # a part of to update the rect attributes to reflect the new tree
        # structure. Use the root's current rect attribute as the
        # starting rectangle for the treemap algorithm.
        root = self
        ...
        return root.update_rectangles(root.rect)
    ...
    def change_size(self, factor: float) -> None:
        ...
        """
        # Change the value of this tree's data_size attribute by <factor> of
        # its current size. Always "round up" the amount to change, so that
        # it's an int, and ensure some change is made.

        new_data_size = math.ceil(abs(self.data_size * factor))
        if factor < 0:
        ...
        # subtree data sizes (the smallest possible value allowed).
        # If the change results in the data_size being less
        # than 1, the data_size should be set to 1. Appropriately update the
        # data_size attribute of ALL TMTrees whose size changes as a result
        # of this change.
        self.data_size += new_data_size
        sum_data_size = 0
        for subtree in self._subtrees:
            sum_data_size += subtree.data_size
        temp_data_size = max(sum_data_size, self.data_size)
        if temp_data_size == sum_data_size:
            if self.data_size < sum_data_size:
                self.data_size = sum_data_size
...
class FileTree(TMTree):
    ...
    def change_size(self, factor: float) -> int:
        self.data_size = math.floor(self.data_size * factor / self.data_size)
...
class ChessTree(TMTree):
    ...
    def __init__(self, move_dict: dict[tuple[str, int], dict],
        ...
        chess_move_subtrees = []
        for (move, value), subtree_dict in move_dict.items():
            subtree = ChessTree(subtree_dict, move, not white_to_play, value)
            chess_move_subtrees.append(subtree)

        TMTree.__init__(self, last_move, chess_move_subtrees, num_games_ended)
